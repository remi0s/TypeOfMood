...
import typeofmood.ime.latin.common.StringUtils;
import typeofmood.ime.latin.define.DebugFlags;
import typeofmood.ime.latin.settings.SettingsValuesForSuggestion;
import typeofmood.ime.latin.utils.AutoCorrectionUtils;
import typeofmood.ime.latin.utils.BinaryDictionaryUtils;
import typeofmood.ime.latin.utils.SuggestionResults;

import static typeofmood.ime.latin.define.DecoderSpecificConstants.SHOULD_AUTO_CORRECT_USING_NON_WHITE_LISTED_SUGGESTION;
import static typeofmood.ime.latin.define.DecoderSpecificConstants.SHOULD_REMOVE_PREVIOUSLY_REJECTED_SUGGESTION;
...
public final class Suggest {
    ...
    private static ArrayList<SuggestedWords.SuggestedWordInfo> getTransformedSuggestedWordInfoList(
        ...
        final ArrayList<SuggestedWords.SuggestedWordInfo> suggestionsContainer = new ArrayList<>(results);
        final int suggestionsCount = suggestionsContainer.size();
        if (isOnlyFirstCharCapitalized || shouldMakeSuggestionsAllUpperCase
                || 0 != trailingSingleQuotesCount) {
            for (int i = 0; i < suggestionsCount; ++i) {
                final SuggestedWords.SuggestedWordInfo wordInfo = suggestionsContainer.get(i);
                final Locale wordLocale = wordInfo.mSourceDict.mLocale;
                final SuggestedWords.SuggestedWordInfo transformedWordInfo = getTransformedSuggestedWordInfo(
    ...
    private static SuggestedWords.SuggestedWordInfo getWhitelistedWordInfoOrNull(
            @Nonnull final ArrayList<SuggestedWords.SuggestedWordInfo> suggestions) {
        if (suggestions.isEmpty()) {
            return null;
        }
        final SuggestedWords.SuggestedWordInfo firstSuggestedWordInfo = suggestions.get(0);
        if (!firstSuggestedWordInfo.isKindOf(SuggestedWords.SuggestedWordInfo.KIND_WHITELIST)) {
            return null;
        }
    ...
    private void getSuggestedWordsForNonBatchInput(final WordComposer wordComposer,
        ...
        final ArrayList<SuggestedWords.SuggestedWordInfo> suggestionsContainer =
        ...
        for (final SuggestedWords.SuggestedWordInfo info : suggestionsContainer) {
        ...
        final int firstOcurrenceOfTypedWordInSuggestions =
                SuggestedWords.SuggestedWordInfo.removeDups(typedWordString, suggestionsContainer);

        final SuggestedWords.SuggestedWordInfo whitelistedWordInfo =
        ...
        if (!isCorrectionEnabled
                ...
                || suggestionResults.first().isKindOf(SuggestedWords.SuggestedWordInfo.KIND_SHORTCUT)) {
            hasAutoCorrection = false;
        } else {
            final SuggestedWords.SuggestedWordInfo firstSuggestion = suggestionResults.first();
        ...
        final SuggestedWords.SuggestedWordInfo typedWordInfo = new SuggestedWords.SuggestedWordInfo(typedWordString,
                "" /* prevWordsContext */, SuggestedWords.SuggestedWordInfo.MAX_SCORE,
    ...
    private void getSuggestedWordsForBatchInput(final WordComposer wordComposer,
        ...
        if (SHOULD_REMOVE_PREVIOUSLY_REJECTED_SUGGESTION
            ...
            final SuggestedWords.SuggestedWordInfo rejected = suggestionsContainer.remove(0);
            suggestionsContainer.add(1, rejected);
        }
        SuggestedWords.SuggestedWordInfo.removeDups(null /* typedWord */, suggestionsContainer);
        ...
        final SuggestedWords.SuggestedWordInfo pseudoTypedWordInfo = suggestionsContainer.isEmpty() ? null
    ...
    private static ArrayList<SuggestedWords.SuggestedWordInfo> getSuggestionsInfoListWithDebugInfo(
            final String typedWord, final ArrayList<SuggestedWords.SuggestedWordInfo> suggestions) {
        final SuggestedWords.SuggestedWordInfo typedWordInfo = suggestions.get(0);
        typedWordInfo.setDebugString("+");
        final int suggestionsSize = suggestions.size();
        final ArrayList<SuggestedWords.SuggestedWordInfo> suggestionsList = new ArrayList<>(suggestionsSize);
        suggestionsList.add(typedWordInfo);
        // Note: i here is the index in mScores[], but the index in mSuggestions is one more
        // than i because we added the typed word to mSuggestions without touching mScores.
        for (int i = 0; i < suggestionsSize - 1; ++i) {
            final SuggestedWords.SuggestedWordInfo cur = suggestions.get(i + 1);
     ...
     */
    private static boolean isAllowedByAutoCorrectionWithSpaceFilter(final SuggestedWords.SuggestedWordInfo info) {
    ...
    /* package for test */ static SuggestedWords.SuggestedWordInfo getTransformedSuggestedWordInfo(
            final SuggestedWords.SuggestedWordInfo wordInfo, final Locale locale, final boolean isAllUpperCase,
        ...
        return new SuggestedWords.SuggestedWordInfo(sb.toString(), wordInfo.mPrevWordsContext,
